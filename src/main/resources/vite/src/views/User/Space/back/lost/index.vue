<template v-if="cards.length>0">  
    ...
    import { getPersonalBackPost } from '@/apis/lost';
    ...
    const queryPosts = async () => {
        getPersonalBackPost(userStore.userInfo.userId).then(res => {
            if(res.msg == 'error'){
    ...
    const loadMore = async () => {
      disabled.value = true;
      const res = await getPersonalBackPost(userStore.userInfo.userId);
      const more = res.data;
