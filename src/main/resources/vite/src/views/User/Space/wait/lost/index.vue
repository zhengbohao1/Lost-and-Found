<template v-if="cards.length>0">  
    ...
    import { getPersonalWaitPost } from '@/apis/lost';
    ...
    const queryPosts = async () => {
        getPersonalWaitPost(userStore.userInfo.userId).then(res => {
            if(res.msg == 'error'){
    ...
    const loadMore = async () => {
      disabled.value = true;
      const res = await getPersonalWaitPost(userStore.userInfo.userId);
      const more = res.data;
