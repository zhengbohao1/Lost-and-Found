        ...
        <div>
          ...
          <div style="text-align: right; margin-top: 10px;">
            <el-popconfirm
              ...
              <template #reference>
                <el-button type="danger" size="medium" :icon="Close" @click.stop>删除</el-button>
              </template>
            </el-popconfirm>
          </div>
        </div>
...
import { queryAllPost } from '@/apis/found';
import { deletePost } from '@/apis/found';
...
const fetchData = async () => {
  try {
    ...
    if (response && response.data) {
      ...
      loading.value = false;
      showErrorInstance.hideErrorMsg(); // 如果请求成功，隐藏错误提示
    } else {
      console.error('Response data is missing or incorrect.');
      showErrorInstance.showErrorMsg('响应数据缺失或不正确'); // 显示错误提示
    }
  } catch (error) {
    console.error('Error fetching data:', error);
    showErrorInstance.showErrorMsg('获取数据时发生错误，请重试'); // 显示错误提示
  }
};
